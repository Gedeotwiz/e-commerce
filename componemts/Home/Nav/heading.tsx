...
export const Heading = () => {
  ...
  return (
    <header className="w-full bg-[#E7F6F2]">
          ...
          <div className="flex items-center gap-3 sm:gap-5">
            {items.map((item, index) => (
              ...
              >
                ...
                <GText className="hidden sm:block text-xs">{item.label}</GText>
              </button>
            ))}
          </div>
      ...
      {menuOpen && (
        <nav className="md:hidden bg-[#E7F6F2] border-t border-[#ccc] flex flex-col items-center py-4 px-5 space-y-3 animate-fadeIn shadow-sm">
          {navLinks.map((label) => {
            const isActive = (label === "Home" && pathname === "/");
            return (
              <GText
